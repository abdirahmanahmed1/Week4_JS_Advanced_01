...
// The event direc//tor needs both the first and last names of each runner for their running bibs.  Combine both the first and last names into a new array called fullName. 
const fllnm = runners.map((allfuln) => {
    return { "First_name": allfuln.first_name, "Last_Name": allfuln.last_name }
})
console.log(fllnm)
...
//let allCaps = [];
//console.log(allCaps); 
const mnk = runners.map((upper) => {
    return upper.first_name.toUpperCase()
})
console.log(mnk)
// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
const largeShirts = runners.filter((bigsize)=>{
    if(bigsize.shirt_size=="L"){
    console.log(`runners that have  a shirt size of are ${bigsize.first_name}`)}
})
...
const  ticketPriceTotal =runners.reduce((acm,item)=>{
return acm+ item.donation;
